...
for state in state_list:
    ...
    if pure_payoff == []:
        ...
        p1, p2, q1, q2 = mixed_strategy_calculation.get_mixed_strategy_equilibrium(q_matrix[state])
    else:
        # pure strategy:
    ...
    v_matrix[state] = nash_equilibrium_calculation.calculate_v(p1, p2, q1, q2, q_matrix[state])
        ...
        for action2 in range(len(r_matrix["S1"][0])):
            for player in range(len(v_matrix["S1"])):
                previous_estimation = v_matrix["S1"][player] * p_matrix[state][action1][action2]["S1"] + v_matrix["S2"][
                    player] * p_matrix[state][action1][action2]["S2"]
...
print("Payoff type:\t", payoff_type)
if count_if_convergence < 10:
    print("Error: Q matrix didn't converge.")
...
fileObject = open('backup_test.json', 'w')

...
def converge_round_bar():
    ...
    pure_3_mixed_0 = [35, 89, 214]  # 线1的纵坐标
    pure_2_mixed_1 = [38, 99, 228]
    pure_1_mixed_2 = [36, 74, 213]
    pure_0_mixed_3 = [39, 63, 219]

    gamma_0_2 = [35, 38, 36, 39]
    gamma_0_5 = [89, 99, 74, 63]
    gamma_0_8 = [214, 228, 213, 219]

    bar_width = 0.2  # 设置柱状图的宽度
    # name_list = ["0.2", "0.5", "0.8"]
    name_list = ["Case 1", "Case 2", "Case 3", "Case 4"]

    x = np.arange(len(name_list))

    # 绘制并列柱状图
    # plt.bar(x, pure_3_mixed_0, bar_width, label='Case 1')
    # plt.bar(x + bar_width, pure_2_mixed_1, bar_width, label='Case 2')
    # plt.bar(x + 2 * bar_width, pure_1_mixed_2, bar_width, label='Case 3')
    # plt.bar(x + 3 * bar_width, pure_0_mixed_3, bar_width, label='Case 4')
    ...
    plt.legend(loc="center right", fontsize=25)  # 图例
    plt.xlabel("Cases", fontsize=30)  # 横坐标名字
    plt.ylabel("Convergent rounds", fontsize=30)  # 纵坐标名字

    plt.xticks(x + 1.5 * bar_width, name_list, fontsize=25)  # 显示x坐标轴的标签,即tick_label,调整位置，使其落在两个直方图中间位置
    plt.yticks(fontsize=25)
    plt.tight_layout()
...
def p_trend(p1_line, p2_line, p3_line, q1_line, q2_line, q3_line, state):
    # 折线图
    fig, ax = plt.subplots(figsize=(8, 6))
    ...
    plt.plot(x, p3_line, '+-', color='r', label="${p_{13}}$")
    plt.plot(x, q3_line, '+-', color='b', label="${q_{23}}$")  # o-:圆形
...
if __name__ == '__main__':
    ...
    with open('backup_data/backup_0.5_pure_3_mixed_0.json', 'r') as f:
        backup = json.load(f)
    ...
    for state in state_list:
        ...
        for i in possibility_history:
            ...
            p3_line.append(i[state]["p3"])
            q1_line.append(i[state]["q1"])
            q2_line.append(i[state]["q2"])
            q3_line.append(i[state]["q3"])
        p_trend(p1_line, p2_line, p3_line, q1_line, q2_line, q3_line, state)
